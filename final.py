...
def results():
    ...
    exp += " ORDER BY movie_id LIMIT 20"
    cur = app.db_connection.cursor()
    cur.execute(exp)
    ms = cur.fetchall()
...
def upload_participants(movie_id):
    ...
    cur.execute("SELECT release_date FROM movie WHERE movie_id = %s", (movie_id,))
    r_date = cur.fetchone()[0]
    ...
    if new:
        ...
        # date of birth too early (earlier than 1850-01-01) or too late (later than movie release_date)
        ddate = datetime.strptime(date, "%Y-%m-%d").date()
        if ddate <= datetime(1850, 1, 1).date() or ddate >= r_date:
            erorr_msg = "Error: Date of Birth earlier than 1850-01-01 or later than the movie release date {}".format(r_date)
            return template('add_participants', page_name="Add Participants", film=film, movie_id=movie_id, profs=profs, vals=vals, err=erorr_msg)
    ...
    if exist:
        # error: haven't chosen an existing person
        if ex_prof_id == "":
            erorr_msg = "Error: you haven't chosen an existing movie professional."
            return template('add_participants', page_name="Add Participants", film=film, movie_id=movie_id, profs=profs, vals=vals, err=erorr_msg)

        # error: chosen person is born after the movie release date
        cur.execute("SELECT date_birth FROM movie_professional WHERE professional_id = %s", (ex_prof_id,))
        db_ex = cur.fetchone()[0]
        if db_ex and db_ex >= r_date:
            erorr_msg = "Error: Date of Birth of the chosen person is later than the movie release date {}".format(r_date)
            return template('add_participants', page_name="Add Participants", film=film, movie_id=movie_id, profs=profs, vals=vals, err=erorr_msg)
